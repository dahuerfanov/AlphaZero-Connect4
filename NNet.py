...
from constants import LR_SGD, MOMENTUM_SGD, COLS, BATCH, EPOCHS
...
class NNet(Module):
    ...
    def __init__(self, name):
        ...
        self.cnn_layers = Sequential(
            Conv2d(in_channels=1, out_channels=256, kernel_size=3, stride=1, padding=1),
            ...
            ReLU(inplace=True),
            Flatten(),
            Linear(in_features=10752, out_features=1024),
            Dropout(p=0.2, inplace=True)
        )

        self.v_layers = Sequential(
            Linear(in_features=1024, out_features=32),
        ...
        self.p_layers = Sequential(
            ...
            Dropout(p=0.1, inplace=True),
            Linear(in_features=64, out_features=COLS),
            Softmax(dim=1),
        )
    ...
    def train_batch(self, x_train, v_train, p_train):
        ...
        return v_loss_train.item(), p_loss_train.item(), \
               ...
               sum([1 - torch.abs(p_output_train[i] - p_train[i]).sum() / COLS for i in range(p_train.size()[0])])
    ...
    def validate(self, x, v, p):
        ...
        return v_loss.item(), p_loss.item(), \
               ...
               sum([1 - torch.abs(p_output[i] - p[i]).sum() / COLS for i in range(p.size()[0])])
    ...
    def run(self, X, Y_v, Y_p):
        ...
        val_v = torch.stack(val_v)
        train_p = torch.stack(train_p)
        val_p = torch.stack(val_p)
        ...
        for _ in range(EPOCHS):
            ...
            acc_v_tr, acc_p_train, acc_v_val, acc_p_val = 0, 0, 0, 0
            div = 0
            for i in range(n_batches):
                if (i + 1) * BATCH > len(train_x):
                    break
                div = i + 1
                loss_v_tr_, loss_p_train_, acc_v_tr_, acc_p_train_ = \
                ...
                acc_v_tr += acc_v_tr_
                acc_p_train += acc_p_train_

            loss_v_val, loss_p_val, acc_v_val, acc_p_val = self.validate(val_x, val_v, val_p)
            ...
            p_train_acc.append(acc_p_train / (BATCH * div))
            v_val_acc.append(acc_v_val / len(val_x))
            p_val_acc.append(acc_p_val / len(val_x))
        ...
        plt.plot(p_train_acc, label='P accuracy (training data)')
        plt.plot(p_val_acc, label='P accuracy (validation data)')
